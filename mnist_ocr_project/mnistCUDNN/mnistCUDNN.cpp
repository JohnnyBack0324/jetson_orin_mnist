...
// 순수 추론(forward) 시간 누적 (ms). classify_example 내부에서 측정.
static double g_total_infer_ms = 0.0;
...
class network_t
{
  ...
  public:
                          ...
                          const Layer_t<value_type>& ip2)
    {
        ...
        // ===== 추론 시간 측정 종료 (cudaDeviceSynchronize 직후) =====
...
{   
    ...
    // 디렉토리 일괄 평가:  ./mnistCUDNN --dir=eval --n=15
    if (checkCmdLineFlag(argc, (const char **)argv, "dir"))
    {
        ...
        if (d == NULL) { FatalError("Cannot open directory"); }
        struct dirent* ent;
        while ((ent = readdir(d)) != NULL)
        {
            std::string f = ent->d_name;
            if (f.size() > 4 && f.substr(f.size()-4) == ".pgm")
                files.push_back(f);
        }
        closedir(d);
        ...
        {
            int req = getCmdLineArgumentInt(argc, (const char **)argv, "n");
            if (req < N) N = req;
        }
        ...
        // ★ I/O + H2D/D2H 통신을 모두 포함한 전체 시간 측정 (조건 부합)
        auto t_start = std::chrono::high_resolution_clock::now();

        for (int i = 0; i < N; i++)
        {
            std::string path = std::string(dir_name) + "/" + files[i];
            int r = mnist.classify_example(path.c_str(), conv1, conv2, ip1, ip2);
            std::cout << "INPUT: " << files[i]
                      << "  Result of classification: " << r << std::endl;
            if (r >= 0 && r < 10) counts[r]++;
        }

        checkCudaErrors(cudaDeviceSynchronize());   // 모든 비동기 연산 완료 보장
        ...
        std::cout << "\nTotal Images : " << N
                  ...
                  << sec << " sec\n";
        std::cout << "Pure Inference Time : " << std::fixed << std::setprecision(3)
                  << g_total_infer_ms << " ms  (forward only, "
                  << (N > 0 ? g_total_infer_ms / N : 0.0) << " ms/image)\n";
        for (int i = 0; i < 10; i++)
            std::cout << "Digit " << i << " : " << counts[i] << " ";
        std::cout << std::endl;
